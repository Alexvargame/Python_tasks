...
class child_ent():
   def __init__(self, child, user):
    ...
    combi_plan = ttk.Combobox(fr_2, values=plans, textvariable=self.PlanString)
   ...
   def save_stat(self):
      ...
      if self.check()==0:
        ...
        self.resultString.set("{},{},{},{},{},{},{},{},{},{},{},{}".format(self.IdString.get(), self.AuthorString.get(),
                                          self.DateString, self.TypesString.get(),
                         self.AdressString.get(), self.RoomsString.get(),
                         self.FloorString.get(), self.FloorsString.get(), self.SquareString.get(),
                         self.PlanString.get(), self.PriceString.get(), self.Text_infoString))
        ...
        with open('_Flats.txt', 'a') as output_file:
           self.text = self.resultString.get()
           output_file.write(self.text)

           
        a = Flats(self.IdString.get(), self.AuthorString.get(), date.today(), self.TypesString.get(), self.AdressCityString.get(),
        ...
        a.pickle('_Flats_class.txt')

        with open('_Flats_class.txt', 'rb') as f:
           while True:
              try:
                 temp = pickle.load(f)
              except EOFError:
