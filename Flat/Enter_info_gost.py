...
class child_ent_gost():
   def __init__(self, child, user):
    ...
    self.lbl_adress_house.grid(row=5, column=0, sticky="w")
    self.lbl_adress_house2.grid(row=6, column=0, sticky="w")
    self.ent_adress_house.grid(row=5, column=1, sticky="w")
    self.ent_adress_house2.grid(row=6, column=1, sticky="w")
    ...
    self.ent_floor = Entry(fr_2, width=10, textvariable=self.FloorString)
    ...
    self.lbl_price.grid(row=5, column=0, sticky="w")
    self.ent_price.grid(row=5, column=1, sticky="w")
    
    self.lbl_text_info = Label(fr_2, text="Информация")
    self.ent_text_info = Text(fr_2, width=20, height=10)
    self.lbl_text_info.grid(row=6, column=0, sticky="e")
   ...
   def save_stat(self):
      ...
      if self.check()==0:
        ...
        self.resultString.set("{},{},{},{},{},{},{},{},{},{},{}".format(self.IdString.get(), self.AuthorString.get(),
                         ...
                         self.FloorString.get(), self.FloorsString.get(), self.SquareString.get(),
                         self.PriceString.get(), self.Text_infoString))
        ...
        with open('_Gostinki.txt', 'a') as output_file:
           self.text = self.resultString.get()
           output_file.write(self.text)

           
        a = Gostinki(self.IdString.get(), self.AuthorString.get(), date.today(), self.TypesString.get(), self.AdressCityString.get(),
                         self.AdressStreetString.get(), self.AdressDistrictString.get(), self.AdressHouseString.get(), self.AdressHouse2String.get(), 
                         self.RoomsString.get(), self.FloorString.get(), self.FloorsString.get(), self.SquareString.get(),
                         self.PriceString.get(), self.Text_infoString)
     
        a.pickle('_Gostinki_class.txt')
        with open('_Gostinki_class.txt', 'rb') as f:
           while True:
              try:
                 temp = pickle.load(f)
              except EOFError:
