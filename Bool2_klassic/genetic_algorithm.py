...
from typing import TypeVar,Tuple, Generic, List, Callable
...
class GeneticAlgorithm(Generic[C]):
    ...
    def __init__(self,initial_population,threshold,max_generations=100, mutation_chance=0.01,
                 crossover_chance=0.7,selection_type=SelectionType.TOURNAMENT):
...
# Примечание: не работает при отрицательных значениях жизнеспособности
    ...
    def _reproduce_and_replace(self):
        ...
        if len(new_population)<len(self._population):
            new_population.pop()
        self._pouplatuion=new_population
