...
class KMeans(Generic[Point]):
    ...
    def __init__(self,k,points):
        ...
        self._clusters=[]
        for _ in range(k):
            rand_points=self._random_point()
            cluster=KMeans.Cluster([],rand_points)
    ...
    def _generate_centroids(self):
        for cluster in self._clusters:
            ...
            for dimension in range(cluster.points[0].num_dimensions):
                dimensions_slice=[p.dimensions[dimension] for p in cluster.points]
                means.append(mean(dimensions_slice))
