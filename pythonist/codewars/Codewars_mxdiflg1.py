...
##           for rep in range(money//max(coins),money//min(coins)+1) if len([item for item in combinations_with_replacement(coins,rep)
##    if sum(item)==money])>0] for items in sublist])
